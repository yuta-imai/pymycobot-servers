...
ステータス: **DRAFT / 合意形成中**（§1 の「要確認」を詰めてから本実装に入る）。
...
要確認（ユーザーに確認したい）:
- **place（置く）先**はどこ？固定の置き場所（座標/トレイ）か、別のボード位置か。pick だけ先行でも可？
- **作業領域の狭さ**は許容する？（当面：到達領域に対象を置く運用）それとも将来ボード/アーム高さを
  再配置して領域拡大も視野に入れる？
- 最終 MCP の**認証配置**：ビジョン MCP（母艦）が Soracom フレームを自前取得する想定で、その env に
  Soracom 認証を置く（＝認証は「soracom-mcp か vision-mcp のどちらか一箇所」）。これで OK か。
- 1サイクルの**期待スループット**（オープンループ前提・サーボで数秒/個で良いか）。
